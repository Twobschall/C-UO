...
namespace Server.Gumps
{
    public class PlayerLevelGump : Gump
    {
		...
		public static void level_OnCommand( CommandEventArgs e )
		{
			...
			{
				e.Mobile.SendMessage("You may not use this sys!");
				return;
			}
			
			if (m_ItemxmlSys.PlayerLevels == false)
			{
				e.Mobile.SendMessage("You may not use this levelsys!");
				return;
			}
		...
		{			
            ...
            {
                CreateSkillList(from, cat, m_ItemxmlSys);
                return;
            }
            else
            {
                CreatePlayerStats(from, m_ItemxmlSys); 
                return;
            }
        ...
        public void CreatePlayerStats(Mobile from, LevelControlSys m_ItemxmlSys)
        {
			xmlplayer = from.Backpack.FindItemByType(typeof(LevelSheet), false) as LevelSheet;
			...
			if(m_ItemxmlSys.LevelStatResetButton == true)
			{
			...
			if(m_ItemxmlSys.LevelSkillResetButton == true)
			{
				AddButton(300, 314, 4005, 4007, GetButtonID(1, 71), GumpButtonType.Reply, 0);
				AddLabel(340, 314, LabelHue2, @"Master Skill Reset - No Confirmation!");
			}
            return;

        }
        public void CreateSkillList(Mobile from, SkillCategory cat, LevelControlSys m_ItemxmlSys)
        {		
            m_From = from;
            m_Cat = cat;
            PlayerMobile pm = from as PlayerMobile;
