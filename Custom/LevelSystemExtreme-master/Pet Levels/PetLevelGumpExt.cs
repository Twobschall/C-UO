...
namespace Server.Gumps
{
	...
	public class PetTarget : Target
	{
		...
		protected override void OnTarget( Mobile from, object target )
		{
			...
			if ( target == from ) //Left here as a sanity check
			...
			{
				var BankBoxVar = pet.FindItemOnLayer(Layer.Bank);
				if (BankBoxVar != null)
				{
					petxml = pet.BankBox.FindItemByType(typeof(PetLevelOrb), false) as PetLevelOrb;
					if (petxml == null)
					{
						from.SendMessage( "This does not work!" );
						return;
					}
					else
	...
	public class PetLevelGump : Gump
	{
		...
		PetLevelOrb m_Petxml;
		PetLevelOrb m_PetAttack;
		...
		{				
			...
			foreach (Item item in map.GetItemsInRange(p,3))
			{
				if (item is LevelControlSysItem)
				{
					...
					cp = (LevelControlSys)XmlAttachExt.FindAttachment(controlitem1, typeof(LevelControlSys));
				}
			}
			if (cp == null){return;}
			if (cp.PlayerLevels == false){return;}
			/* LevelSystemExt */
			
			m_Pet = target;
			PetLevelOrb m_Petxml = null;
			
			var BankBoxVar = m_Pet.FindItemOnLayer(Layer.Bank);
			if (BankBoxVar == null)
				return;
			
			m_Petxml = m_Pet.BankBox.FindItemByType(typeof(PetLevelOrb), false) as PetLevelOrb;	
			if (m_Petxml == null)
				return;
		
			m_PetAttack = m_Pet.BankBox.FindItemByType(typeof(PetLevelOrb), false) as PetLevelOrb;
			if (m_PetAttack == null)
				return;
