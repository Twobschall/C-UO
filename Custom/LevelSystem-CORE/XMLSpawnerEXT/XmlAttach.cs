...
namespace Server.Engines.XmlSpawnerExtMod
{
	...
	public class ASerial
	{
		...
		public static ASerial NewSerial()
		{
			...
			while (XmlAttachExt.AllAttachments.ContainsKey(newserialno))
			{
				newserialno++;
	...
	public class XmlAttachExt
	{
		...
		public static void HashSerial(ASerial key, XmlAttachmentExt o)
		{
			if (key.Value != 0)
			{
		...
		public static Dictionary<Item, List<XmlAttachmentExt>> ItemAttachments = new Dictionary<Item, List<XmlAttachmentExt>>();
		public static Dictionary<Mobile, List<XmlAttachmentExt>> MobileAttachments = new Dictionary<Mobile, List<XmlAttachmentExt>>();
		public static Dictionary<int, XmlAttachmentExt> AllAttachments = new Dictionary<int, XmlAttachmentExt>();
		private static List<XmlAttachmentExt> UnassignedAttachments = new List<XmlAttachmentExt>();

		public static bool HasAttachments(object o)
		{
			if (o == null) return false;

			List<XmlAttachmentExt> alist;
			if (o is Item && ItemAttachments.TryGetValue((Item)o, out alist))//.Contains(o))
			{
				...
				foreach (XmlAttachmentExt a in alist)
				{
		...
		public static XmlAttachmentExt[] Values
		{
			get
			{
				XmlAttachmentExt[] valuearray = new XmlAttachmentExt[XmlAttachExt.AllAttachments.Count];
				XmlAttachExt.AllAttachments.Values.CopyTo(valuearray, 0);
				return valuearray;
			}
		}
		...
		public static void Initialize()
		{
			...
			foreach (XmlAttachmentExt a in UnassignedAttachments)
			{
				...
				XmlAttachExt.HashSerial(serial, a);
			}
			...
			CommandSystem.Register( "ItemAttExt", AccessLevel.GameMaster, new CommandEventHandler( ListItemAttachments_OnCommand ) );
			//CommandSystem.Register( "MobAtt", AccessLevel.GameMaster, new CommandEventHandler( ListMobileAttachments_OnCommand ) );
			CommandSystem.Register("GetAttExt", AccessLevel.GameMaster, new CommandEventHandler(GetAttachments_OnCommand));
			...
			CommandSystem.Register("AvailAttExt", AccessLevel.GameMaster, new CommandEventHandler(ListAvailableAttachments_OnCommand));
		}
		...
		public class AddAttCommand : BaseCommand
		{
			public AddAttCommand()
			{
				...
				Commands = new string[] { "AddAttExt" };
				ObjectTypes = ObjectTypes.Both;
				Usage = "AddAttExt type [args]";
			...
			public override void ExecuteList(CommandEventArgs e, ArrayList list)
			{
				if (e != null && list != null && e.Length >= 1)
				{
					...
					if (attachtype != null && attachtype.IsSubclassOf(typeof(XmlAttachmentExt)))
					{
						...
						for (int i = 0; i < list.Count; ++i)
						{
							...
							XmlAttachmentExt o = (XmlAttachmentExt)XmlSpawnerExt.CreateObjectExt(attachtype, args, false, true);
							...
							if (XmlAttachExt.AttachTo(null, list[i], o, true))
							{
								if (list.Count < 10)
								{
		...
		public class DelAttCommand : BaseCommand
		{
			public DelAttCommand()
			{
				AccessLevel = AccessLevel.GameMaster;
				Supports = CommandSupport.All;
				Commands = new string[] { "DelAttExt" };
				ObjectTypes = ObjectTypes.Both;
				Usage = "DelAttExt type";
			...
			public override void ExecuteList(CommandEventArgs e, ArrayList list)
			{
				if (e != null && list != null && e.Length >= 1)
				{
					...
					if (attachtype != null && attachtype.IsSubclassOf(typeof(XmlAttachmentExt)))
					{
						...
						for (int i = 0; i < list.Count; ++i)
						{
							List<XmlAttachmentExt> alist = XmlAttachExt.FindAttachments(list[i], attachtype);
		...
		public static void CleanUp()
		{
			...
			foreach (XmlAttachmentExt a in XmlAttachExt.Values)
			{
		...
		{
			if (XmlAttachExt.MobileAttachments == null && XmlAttachExt.ItemAttachments == null) return;

			CleanUp();

			if (!Directory.Exists("Saves/ExtXmlAttachments"))
				Directory.CreateDirectory("Saves/ExtXmlAttachments");

			string filePath = Path.Combine("Saves/ExtXmlAttachments", "Attachments.bin");        // the attachment serializations
			string imaPath = Path.Combine("Saves/ExtXmlAttachments", "Attachments.ima");         // the item/mob attachment tables
			string fpiPath = Path.Combine("Saves/ExtXmlAttachments", "Attachments.fpi");        // the file position indices
			...
			if (writer != null && imawriter != null && fpiwriter != null)
			{
				...
				XmlAttachExt.FullDefrag();

				// save the attachments themselves
				if (XmlAttachExt.AllAttachments != null)
				{
					writer.Write(XmlAttachExt.AllAttachments.Count);

					XmlAttachmentExt[] valuearray = new XmlAttachmentExt[XmlAttachExt.AllAttachments.Count];
					XmlAttachExt.AllAttachments.Values.CopyTo(valuearray, 0);

					int[] keyarray = new int[XmlAttachExt.AllAttachments.Count];
					XmlAttachExt.AllAttachments.Keys.CopyTo(keyarray, 0);
					...
					for (int i = 0; i < keyarray.Length; i++)
					{
						...
						XmlAttachmentExt a = valuearray[i];
				...
				// mobile attachments
				if (XmlAttachExt.MobileAttachments != null)
				{
					imawriter.Write(XmlAttachExt.MobileAttachments.Count);

					List<XmlAttachmentExt>[] valuearray = new List<XmlAttachmentExt>[XmlAttachExt.MobileAttachments.Count];
					XmlAttachExt.MobileAttachments.Values.CopyTo(valuearray, 0);

					Mobile[] keyarray = new Mobile[XmlAttachExt.MobileAttachments.Count];
					XmlAttachExt.MobileAttachments.Keys.CopyTo(keyarray, 0);
					...
					for (int i = 0; i < keyarray.Length; i++)
					{
						...
						List<XmlAttachmentExt> alist = valuearray[i];
				...
				// item attachments
				if (XmlAttachExt.ItemAttachments != null)
				{
					imawriter.Write(XmlAttachExt.ItemAttachments.Count);

					List<XmlAttachmentExt>[] valuearray = new List<XmlAttachmentExt>[XmlAttachExt.ItemAttachments.Count];
					XmlAttachExt.ItemAttachments.Values.CopyTo(valuearray, 0);

					Item[] keyarray = new Item[XmlAttachExt.ItemAttachments.Count];
					XmlAttachExt.ItemAttachments.Keys.CopyTo(keyarray, 0);
		...
		public static void Load()
		{
			...
			if (reader != null && imareader != null && fpireader != null)
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					XmlAttachmentExt o = null;
					try
					{
						o = (XmlAttachmentExt)Activator.CreateInstance(Type.GetType(valuetype), new object[] { serialno });
					}
					catch
					{
						skip = true;
				...
				for (int i = 0; i < count; i++)
				{
					...
					for (int j = 0; j < nattach; j++)
					{
						...
						XmlAttachmentExt o = FindAttachmentBySerial(serialno.Value);
		...
		public static void CheckOnBeforeKill(Mobile m_killed, Mobile m_killer)
		{
			...
			{
				// check the killer
				List<XmlAttachmentExt> alist = XmlAttachExt.FindAttachments(m_killer);
				if (alist != null)
				{
					foreach (XmlAttachmentExt a in alist)
					{
				...
				if (equiplist != null)
				{
					foreach (Item i in equiplist)
					{
						...
						alist = XmlAttachExt.FindAttachments(i);
						if (alist != null)
						{
							foreach (XmlAttachmentExt a in alist)
							{
			...
			{
				// check the killed
				List<XmlAttachmentExt> alist = XmlAttachExt.FindAttachments(m_killed);
				if (alist != null)
				{
					foreach (XmlAttachmentExt a in alist)
					{
			...
			if (from.Map != null)
			{
				...
				if (itemlist != null)
				{
					foreach (Item i in itemlist)
					{
						...
						List<XmlAttachmentExt> alist = XmlAttachExt.FindAttachments(i);
						if (alist != null)
						{
							foreach (XmlAttachmentExt a in alist)
							{
		...
		public static void EventSink_Speech(SpeechEventArgs args)
		{
			...
			List<XmlAttachmentExt> alist = XmlAttachExt.FindAttachments(from);
			if (alist != null)
			{
				foreach (XmlAttachmentExt a in alist)
				{
		...
		public static XmlAttachmentExt FindAttachmentOnMobile(Mobile from, Type type, string name)
		{
		...
		private class AttachTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				XmlAttachExt.Defrag(targeted);

				List<XmlAttachmentExt> plist = XmlAttachExt.FindAttachments(targeted, type);
				...
				switch (m_set)
				{
					case "add":
						...
						if (attachtype != null && attachtype.IsSubclassOf(typeof(XmlAttachmentExt)))
						{
							...
							o = (XmlAttachmentExt)XmlSpawnerExt.CreateObjectExt(attachtype, args, false, true);
						}

						if (o != null)
						{
							//o.Name = aname;
							if (XmlAttachExt.AttachTo(from, targeted, o, true))
					...
					case "get":
						/*
							foreach(XmlAttachmentExt p in plist)
							{
							...
							*/
						from.SendGump(new XmlGetAttGumpExt(from, targeted, 0, 0));
		...
		[Usage("GetAttExt [type/serialno [name]]")]
		...
		public static void GetAttachments_OnCommand(CommandEventArgs e)
		{
			...
			if (e.Arguments.Length > 0)
			{
				...
				if (c >= '0' && c <= '9')
				{
					...
					XmlAttachmentExt a = FindAttachmentBySerial(ser);
					if (a != null)
					{
		...
		public static void ListItemAttachments_OnCommand(CommandEventArgs e)
		{
			...
			XmlAttachExt.FullDefrag(ItemAttachments);
			...
			for (int i = 0; i < itemarray.Length; i++)
			{
				...
				List<XmlAttachmentExt> list = FindAttachments(itemarray[i]);

				if (list != null)
				{
					foreach (XmlAttachmentExt a in list)
					{
						if (a != null && !a.Deleted)
		...
		public static void ListMobileAttachments_OnCommand(CommandEventArgs e)
		{
			...
			XmlAttachExt.FullDefrag(MobileAttachments);
			...
			for (int i = 0; i < mobilearray.Length; i++)
			{
				...
				List<XmlAttachmentExt> list = FindAttachments(mobilearray[i]);
		...
		{
			List<Type> attachtypes = Match(typeof(XmlAttachmentExt));

			string parmliststr = null;

			foreach (Type attachtype in attachtypes)
			{
				// get all constructors derived from the XmlAttachmentExt class
		...
		public static void RevealAttachments(Mobile from, object o)
		{
			...
			List<XmlAttachmentExt> plist = XmlAttachExt.FindAttachments(o);
		...
		public static bool AttachTo(object o, XmlAttachmentExt attachment)
		{
			return AttachTo(null, o, attachment, true);
		}

		public static bool AttachTo(object from, object o, XmlAttachmentExt attachment)
		{
			return AttachTo(from, o, attachment, true);
		}

		public static bool AttachTo(object o, XmlAttachmentExt attachment, bool first)
		{
			return AttachTo(null, o, attachment, first);
		}

		private static bool AttachTo(object from, object o, XmlAttachmentExt attachment, bool first)
		{
			if (attachment == null || o == null) return false;

			Item it=null;
			Mobile mob=null;
			List<XmlAttachmentExt> attachmententry;
			if (o is Item)
			{
				it=(Item)o;
				XmlAttachExt.Defrag(ItemAttachments, it);
				attachmententry = FindAttachments(it, true);
			}
			else if (o is Mobile)
			{
				mob=(Mobile)o;
				XmlAttachExt.Defrag(MobileAttachments, mob);
			...
			if (attachmententry != null)
			{
				...
				foreach (XmlAttachmentExt i in attachmententry)
				{
			...
			else
			{
				...
				attachmententry = new List<XmlAttachmentExt>();
		...
		public static List<XmlAttachmentExt> FindAttachments(object o)
		{
			return FindAttachments(o, null, null);
		}

		public static List<XmlAttachmentExt> FindAttachments(Item o)
		{
			return FindAttachments(ItemAttachments, o, null, null, false);
		}

		public static List<XmlAttachmentExt> FindAttachments(Mobile o)
		{
			return FindAttachments(MobileAttachments, o, null, null, false);
		}

		public static List<XmlAttachmentExt> FindAttachments(object o, Type type)
		{
			return FindAttachments(o, type, null);
		}

		public static List<XmlAttachmentExt> FindAttachments(Item o, Type type)
		{
			return FindAttachments(ItemAttachments, o, type, null);
		}

		public static List<XmlAttachmentExt> FindAttachments(Mobile o, Type type)
		{
			return FindAttachments(MobileAttachments, o, type, null);
		}

		public static List<XmlAttachmentExt> FindAttachments(object o, Type type, string name)
		{
			if (o is Item)
			{
		...
		public static List<XmlAttachmentExt> FindAttachments(Item o, Type type, string name)
		{
			return FindAttachments(ItemAttachments, (Item)o, type, name, false);
		}

		public static List<XmlAttachmentExt> FindAttachments(Mobile o, Type type, string name)
		{
			return FindAttachments(MobileAttachments, (Mobile)o, type, name, false);
		}

		public static List<XmlAttachmentExt> FindAttachments(Item o, bool original)
		{
			return FindAttachments(ItemAttachments, o, null, null, original);
		}

		public static List<XmlAttachmentExt> FindAttachments(Mobile o, bool original)
		{
			return FindAttachments(MobileAttachments, o, null, null, original);
		}

		public static List<XmlAttachmentExt> FindAttachments(Dictionary<Item, List<XmlAttachmentExt>> attachments, Item o, Type type, string name)
		{
			return FindAttachments(attachments, o, type, name, false);
		}

		public static List<XmlAttachmentExt> FindAttachments(Dictionary<Mobile, List<XmlAttachmentExt>> attachments, Mobile o, Type type, string name)
		{
			return FindAttachments(attachments, o, type, name, false);
		}

		public static List<XmlAttachmentExt> FindAttachments(Dictionary<Item, List<XmlAttachmentExt>> attachments, Item o, Type type, string name, bool original)
		{
			if (o == null || attachments == null || o.Deleted) return null;

			List<XmlAttachmentExt> list;
			...
			else
			{
				...
				List<XmlAttachmentExt> newlist = new List<XmlAttachmentExt>();

				foreach (XmlAttachmentExt i in list)
				{
		...
		public static List<XmlAttachmentExt> FindAttachments(Dictionary<Mobile, List<XmlAttachmentExt>> attachments, Mobile o, Type type, string name, bool original)
		{
			if (o == null || attachments == null || o.Deleted) return null;
		...
		public static XmlAttachmentExt FindAttachment(object o)
		{
			return FindAttachment(o, null, null);
		}

		public static XmlAttachmentExt FindAttachment(Item o)
		{
			return FindAttachment(o, null, null);
		}

		public static XmlAttachmentExt FindAttachment(Mobile o)
		{
			return FindAttachment(o, null, null);
		}

		public static XmlAttachmentExt FindAttachment(object o, Type type)
		{
			return FindAttachment(o, type, null);
		}

		public static XmlAttachmentExt FindAttachment(Item o, Type type)
		{
			return FindAttachment(o, type, null);
		}

		public static XmlAttachmentExt FindAttachment(Mobile o, Type type)
		{
			return FindAttachment(o, type, null);
		}

		public static XmlAttachmentExt FindAttachment(object o, Type type, string name)
		{
			if (o is Item)
			{
		...
		public static XmlAttachmentExt FindAttachment(Item o, Type type, string name)
		{
			return FindAttachment(ItemAttachments, o, type, name);
		}

		public static XmlAttachmentExt FindAttachment(Mobile o, Type type, string name)
		{
			return FindAttachment(MobileAttachments, o, type, name);
		}

		public static XmlAttachmentExt FindAttachment(Dictionary<Item, List<XmlAttachmentExt>> attachments, Item o, Type type)
		{
			return FindAttachment(attachments, o, type, null);
		}

		public static XmlAttachmentExt FindAttachment(Dictionary<Mobile, List<XmlAttachmentExt>> attachments, Mobile o, Type type)
		{
			return FindAttachment(attachments, o, type, null);
		}

		public static XmlAttachmentExt FindAttachment(Dictionary<Item, List<XmlAttachmentExt>> attachments, Item o, Type type, string name)
		{
			List<XmlAttachmentExt> list;
		...
		public static XmlAttachmentExt FindAttachment(Dictionary<Mobile, List<XmlAttachmentExt>> attachments, Mobile o, Type type, string name)
		{
			List<XmlAttachmentExt> list;
			if(o == null || attachments == null || o.Deleted || !attachments.TryGetValue(o, out list) || list==null) return null;
		...
		public static XmlAttachmentExt FindAttachmentBySerial(int serialno)
		{
		...
		private static void FullDefrag(Dictionary<Item, List<XmlAttachmentExt>> attachments)
		{
		...
		private static void FullDefrag(Dictionary<Mobile, List<XmlAttachmentExt>> attachments)
		{
		...
		private static void FullSerialDefrag(Dictionary<int, XmlAttachmentExt> attachments)
		{
			...
			for (int i = 0; i < keyarray.Length; i++)
			{
				XmlAttachmentExt a = attachments[keyarray[i]];
		...
		private static void SerialDefrag(XmlAttachmentExt a)
		{
			if (a != null && a.Deleted)
		...
		private static void Defrag(Dictionary<Item, List<XmlAttachmentExt>> attachments, Item o)
		{
			if (o == null || attachments == null) return;

			bool removeall = false;
			List<XmlAttachmentExt> list = null;
		...
		private static void Defrag(Dictionary<Mobile, List<XmlAttachmentExt>> attachments, Mobile o)
		{
			if (o == null || attachments == null) return;
			bool removeall = false;
			List<XmlAttachmentExt> list = null;
		...
		public static bool CheckCanEquip(Item item, Mobile from)
		{
			...
			List<XmlAttachmentExt> attachments = FindAttachments(item);

			if (attachments != null)
			{
				foreach (XmlAttachmentExt a in attachments)
				{
					if (a != null && !a.Deleted)
		...
		public static void OnWeaponHit(BaseWeapon weapon, Mobile attacker, Mobile defender, int damage)
		{
			...
			List<XmlAttachmentExt> attachments = FindAttachments(weapon);
		...
		public static int OnArmorHit(Mobile attacker, Mobile defender, Item armor, BaseWeapon weapon, int damage)
		{
			...
			if (armor != null)
			{
				List<XmlAttachmentExt> attachments = FindAttachments(armor);
		...
		public static void AddAttachmentProperties(object parent, ObjectPropertyList list)
		{
			...
			List<XmlAttachmentExt> plist = XmlAttachExt.FindAttachments(parent);
			if (plist != null && plist.Count > 0)
			{
				bool more=plist.Count>1;
				for (int i = 0; i < plist.Count; i++)
				{
					XmlAttachmentExt a = plist[i];
				...
				else
				{
					...
					if (s.IsMobile)
					{
						...
						if (m != null && !m.Deleted)
						{
							...
							List<XmlAttachmentExt> fromlist = FindAttachments(from);
							if (fromlist != null)
							{
								foreach (XmlAttachmentExt a in fromlist)
								{
									if (a != null && !a.Deleted)
									{
							...
							List<XmlAttachmentExt> alist = FindAttachments(m);
							if (alist != null)
							{
								foreach (XmlAttachmentExt a in alist)
								{
					...
					else if (s.IsItem)
					{
						...
						if (item != null && !item.Deleted)
						{
							...
							// get attachments on the mob
							List<XmlAttachmentExt> alist = FindAttachments(item);
							if (alist != null)
							{
								foreach (XmlAttachmentExt a in alist)
								{
		...
		public class ErrorReporter
		{
			...
			{
				Console.Write("XmlSpawnerExtMod Attachment error: Sending email...");
				...
				message.Subject = "Automated XmlSpawnerExtMod Attachment Error Report";

				message.Body = "Automated XmlSpawnerExtMod Attachment Report. See attachment for details.";
			...
			{
				Console.Write("\nXmlSpawnerExtMod Attachment Error:\n{0}\nGenerating report...", error);
				...
				try
				{
					...
					using (StreamWriter op = new StreamWriter(filePath))
					{
						...
						op.WriteLine("XmlSpawnerExtMod Attachment Error Report");
						op.WriteLine("===================");
						op.WriteLine();
						op.WriteLine("RunUO Version {0}.{1}.{3}, Build {2}", ver.Major, ver.Minor, ver.Revision, ver.Build);
						op.WriteLine("Operating System: {0}", Environment.OSVersion);
						op.WriteLine(".NET Framework: {0}", Environment.Version);
//						op.WriteLine("XmlSpawnerExtMod: {0}", XmlSpawnerExt.Version);
						...
						foreach (DeserErrorDetails s in XmlAttachExt.desererror)
						{
