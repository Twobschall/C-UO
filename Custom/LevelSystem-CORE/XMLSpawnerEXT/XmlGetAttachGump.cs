...
{
	public class XmlGetAttGumpExt : Gump
	{
		...
		private List<XmlAttachmentExt> m_SearchList;
		...
		private bool TestAge(object o)
		{
			...
			if(o is XmlAttachmentExt){
				XmlAttachmentExt a = (XmlAttachmentExt)o;
		...
		private List<XmlAttachmentExt> Search(object target, out string status_str)
		{
			status_str = null;
			List<XmlAttachmentExt> newarray = new List<XmlAttachmentExt>();
			...
			List<XmlAttachmentExt> attachments = XmlAttachExt.FindAttachments(target);

			// do the search through attachments
			if(attachments != null)
			foreach(XmlAttachmentExt i in attachments)
			{
				bool hastype = false;
				bool hasname = false;
		...
		public XmlGetAttGumpExt(Mobile from, object targeted, int x, int y) : this(from, targeted, true, false,
		false, false, false,
		null,null, false, 0,
		null, -1, 0,
		false,false,
		...
		public XmlGetAttGumpExt( Mobile from, object targeted, bool firststart, bool descend,
			...
			List<XmlAttachmentExt> searchlist, int selected, int displayfrom,
			...
			bool selectall, bool [] selectionlist, int X, int Y ) : base( X,Y )
		{
			...
			for ( int i = 0;  i < MaxEntries; i++ )
			{
				...
				if(o is XmlAttachmentExt){
					XmlAttachmentExt a = m_SearchList[index];
		...
		private void DoShowProps(int index)
		{
			...
			{
				XmlAttachmentExt x = m_SearchList[index];
		...
		private class ListTypeSorter : IComparer<XmlAttachmentExt>
		{
		  private bool Dsort;
			...
			public int Compare( XmlAttachmentExt x, XmlAttachmentExt y )
			{
				 string xstr=null;
				 string ystr=null;
				 string str=null;
		...
		private class ListNameSorter : IComparer<XmlAttachmentExt>
		{
		  private bool Dsort;
		...
		{
			state.Mobile.SendGump( new XmlGetAttGumpExt(this.m_From, this.m_TargetObject, false, this.Descendingsort,
				...
				default:
				{
					...
					if(info.ButtonID >= 5000 && info.ButtonID < 5000+ MaxEntries){
						...
						if(m_SelectionList != null && i >= 0  && i < m_SelectionList.Length && !SelectAll){
							// only toggle the selection list entries for things that actually have entries
							if(m_SearchList != null && (m_SearchList.Count - DisplayFrom > i)) {
								XmlAttachmentExt a = m_SearchList[i+DisplayFrom];
								if(a != null)
								{
	...
	{
		private List<XmlAttachmentExt> SearchList;
		...
		public XmlConfirmDeleteGump(Mobile from, object target, List<XmlAttachmentExt> searchlist, bool [] selectedlist, int displayfrom, bool selectall, int allcount) : base ( 0, 0 )
		{
					...
					{    // accept
						for(int i = 0;i < SearchList.Count;i++){
							...
							if((index >= 0 && index < SelectedList.Length && SelectedList[index] == true) || selectAll){
								XmlAttachmentExt o = SearchList[i];
						...
						// refresh the gump
						state.Mobile.CloseGump(typeof(XmlGetAttGumpExt));
						state.Mobile.SendGump(new XmlGetAttGumpExt(state.Mobile,m_target,0,0));
					}
					break;
				}
			}
