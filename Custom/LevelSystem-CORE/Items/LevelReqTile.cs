...
{ 
	...
	{ 
		...
		{
			LevelSheet gate = null;
			gate = from.Backpack.FindItemByType(typeof(LevelSheet), false) as LevelSheet;
			if (gate == null)
				return false;
            if (RequiredExactLevel == true)
			{
				...
				{
					from.SendMessage( "You do not meet the level requirement for this step." );
					return false;
				}
			}
