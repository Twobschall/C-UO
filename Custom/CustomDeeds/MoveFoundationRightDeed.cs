...
{
    public class MoveFoundationRightDeed : Item
    {
        [Constructable]
        public MoveFoundationRightDeed() : base(0x14F0)
        {
            Weight = 1.0;
            LootType = LootType.Blessed;
            Name = "Move Foundation Right 2 Spaces Deed";
        }

        public MoveFoundationRightDeed(Serial serial) : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.SendMessage("Target the house that you want to move right 2 spaces.");
                from.Target = new MoveFoundationRightTarget(this, from);
            }
        }

        private class MoveFoundationRightTarget : Target
        {
            private readonly MoveFoundationRightDeed m_Script;
            private readonly Mobile m_Player;

            public MoveFoundationRightTarget(MoveFoundationRightDeed script, Mobile player) : base(10, false, TargetFlags.None)
            {
                m_Script = script;
                m_Player = player;
            }
                ...
                if (point != null)
                {
                    ...
                    if (house != null)
                    {
                        ...
                        if (house.Owner == from)
                        {
                            ...
                            from.SendMessage("The house has been moved right 2 blocks.");
                            // Delete the deed
                            m_Script.Delete();
                        }
                        else
